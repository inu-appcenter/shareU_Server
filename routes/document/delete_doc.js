...
router.post('/',(req,res)=>{
    ...
    let sqlfile_search = 'SELECT fileName FROM file WHERE documentKey=?';
      db.query(sqlfile_search,[documentKey], async (err,results) =>{
      if(err){
        console.log(err)
        ...
        fs.unlink(`./uploads/${fileName_old}`,async (err)=>{ // 원래 있던 파일 삭제
          if(err) throw err;
          else{
    ...
    // let sqljoin = 'SELECT * FROM document AS doc LEFT JOIN file AS fi ON doc.documentKey = fi.documentKey LEFT JOIN point AS poi ON doc.documentKey = poi.documentKey WHERE fi.documentKey=?'
           ...
           await db.query(sqljoin_Delete, [documentKey],async (err, rows) => { 
              if (err) {
              console.log(err);
              return res.sendStatus(400);
            }else {
              
              await res.status(200).json({ans:true});
            }
              });
          }
          });
